...
class TestFields(unittest.TestCase):
    ...
    def testNodeField(self):
        ...
        self.assertTrue(
            isinstance(obj.missing, TestSubobject),
    ...
    def testStringField(self):
        ...
        self.assertEqual(
            obj.node.xpath("string(ex:missing)", namespaces=self.namespaces),
            "over there",
        )
        # in attrib
        ...
        self.assertEqual(
            obj.node.xpath("string(bar/missing)"), "pining (for the fjords)"
        )

        # in subelement which is itself missing
        obj.multilevel_missing = "so, so gone"
        self.assertEqual(
            obj.node.xpath("string(missing_parent/missing_child)"), "so, so gone"
        )
    ...
    def testFormattedDateTimeField(self):
        ...
        obj.date = today
        self.assertEqual(
            obj.node.xpath("string(datetime)"), today.strftime("%Y-%m-%dT%H:%M:%S")
        )
    ...
    def testPredicatedSetting(self):
        ...
        self.assertEqual(
            obj.node.xpath('string(pred[@a="foo"]/pred[@b="bar"])'), "test"
        )
...
class TestNodeList(unittest.TestCase):
    ...
    def test_index_checking(self):
        ...
        self.assertRaises(
            AssertionError, self.obj.str.__setitem__, slice(0, 10), ["val"]
        )
    ...
    def test_del(self):
        ...
        self.assertEqual(
            1,
            len(self.obj.str),
            "StringListField length should be 1 after deletion, got %d"
            % len(self.obj.str),
        )
        ...
        self.assertEqual(
            1,
            len(int_list),
            "IntegerListField length should be 1 after deletion, got %d"
            % len(int_list),
        )
        ...
        self.assertEqual(
            0,
            len(self.obj.nodes),
            "NodeListField length should be 0 after deletion, got %d"
            % len(self.obj.nodes),
        )
    ...
    def test_append(self):
        ...
        self.assertEqual(
            3, len(self.obj.str), "length of StringListField is 3 after appending value"
        )
        ...
        int_list.append(9)
        self.assertEqual(
            3, len(int_list), "length of IntegerListField is 3 after appending value"
        )
    ...
    def test_remove(self):
        ...
        letters.remove("a")
        self.assertNotEqual(
            "a", letters[0], "first letter is no longer 'a' after removing 'a'"
        )
        self.assertEqual(
            1,
            letters.index("a"),
            "index for 'a' is 1 after removing 'a' - expected 1, got %d"
            % letters.index("a"),
        )
        ...
        self.assertTrue(
            node not in self.obj.nodes, "node is not in NodeList after remove"
        )
    ...
    def test_pop(self):
        ...
        self.assertEqual(
            "y",
            last,
            "pop with no index returned last letter in list - expected 'y', got '%s'"
            % last,
        )
        self.assertTrue(
            "y" not in self.obj.letters,
        ...
        self.assertEqual(
            "a",
            first,
            "pop with index 0 returned first letter in list - expected 'a', got '%s'"
            % first,
        )
        self.assertNotEqual(
            "a", self.obj.letters[0], "first letter is no longer 'a' after pop index 0"
        )
        ...
        self.assertEqual(
            ["side-a", "side-b"], node.parts, "popped node has expected parts"
        )
    ...
    def test_insert(self):
        ...
        self.assertEqual(
            "99",
            letters[-1],
            "last item in letters should be '99' after inserting at end; got '%s'"
            % letters[-1],
        )
        ...
        self.assertEqual(
            "z",
            self.obj.empty[0],
            "empty[0] should be 'z' after inserting 'z' at 0, got '%s'"
            % self.obj.empty[0],
        )
